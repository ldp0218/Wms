...
class WlpdActivity : BaseActivity(R.layout.activity_pd) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        RetrofitManager.instance.pdList(0)
                .enqueue(object : BaseCallback<ResponseList<Pd>>(this@WlpdActivity) {
                    override fun successData(data: ResponseList<Pd>) {
                        ...
                        pdAdapter = object : CommonAdapter<Pd>(pdList, R.layout.listview_pd_item) {
                            override fun convert(holder: ViewHolder, t: Pd, position: Int) {
                                ...
                                holder.setOnClickListener(R.id.layout_pd, View.OnClickListener { _ ->
                                    // 点击查看详情
                                    val p = pdList!![position]
                                    val intent = Intent(this@WlpdActivity, KcpdActivity::class.java)
                                    intent.putExtra("type", 0)  // 设置标识
                                    intent.putExtra("pdId", p.id)
                                    intent.putExtra("ckId", p.ck_id)
                                    intent.putExtra("ckName", p.ck_name)
                                    intent.putExtra("pdDate", p.pd_date)
                        ...
                        refreshLayout.setOnLoadmoreListener { smartLayout ->    // 加载更多
                            if (totalPages > page) {
                                smartLayout.layout.postDelayed({
                                    RetrofitManager.instance.pdList(0, page + 1)
                                            .enqueue(object : BaseCallback<ResponseList<Pd>>(this@WlpdActivity) {
                                                override fun successData(data: ResponseList<Pd>) {
                                                    pdList!!.addAll(data.items)
                                                    pdAdapter.notifyDataSetChanged()
